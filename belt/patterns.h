...
//          leds[i] = CHSV((int)(i + offset)%0xFF, 0xFF, 0xFF);
//        }
//       if (lead < NUM_LEDS) {
//      for (int i = 0; i < NUM_LEDS; ++i) {
//        if (lead%NUM_LEDS == i) {
          leds[lead%NUM_LEDS] = CHSV((int)(lead + offset)%0xFF, 0xFF, 0xFF);
//        } else {
//          leds[lead%NUM_LEDS] = leds[lead%NUM_LEDS];
//        }
//      }
      lead++;
      offset += 0.7;
...
//      const int reads = 10;
//      
//      int sensor = analogRead(MIC_PIN);
//      avg = ((reads - 1) * avg + sensor) / (float)reads;
//      logf("avg = %f", avg);
//      for (int x = 0; x < 50 * avg; ++x) {
//        for (int y = 0; y < 8; ++y) {
//          leds[8 * x + y] = CRGB(0x7F, 0x7F, 0x7F);
//        }
//      }
